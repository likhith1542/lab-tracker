    ...
    <div className="flex-1 overflow-y-auto" style={{ paddingBottom: 100 }}>
      ...
      <div className="px-5 pt-6 pb-4">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-extrabold gradient-text-cyan" style={{ fontFamily: 'Syne' }}>Experiments</h1>
          <button
            onClick={() => setShowCreate(true)}
            className="p-2.5 rounded-xl transition-all active:scale-90"
            style={{ background: 'linear-gradient(135deg, var(--accent-cyan), var(--accent-green))' }}
          >
            <Plus size={18} style={{ color: '#0a0a0f' }} strokeWidth={2.5} />
          </button>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {FILTERS.map(f => (
            <button
              ...
              style={{
                background: filter === f ? 'rgba(0,229,255,0.12)' : 'var(--bg-card)',
                borderColor: filter === f ? 'var(--accent-cyan)' : 'var(--bg-border)',
            ...
            >
              {f} {f !== 'all' && `(${experiments.filter(e => e.status === f).length})`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="font-semibold mb-1" style={{ fontFamily: 'Syne' }}>
              {search ? 'No experiments found' : `No ${filter === 'all' ? '' : filter} experiments yet`}
            </p>
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
              {!search && filter === 'all' && 'Start your first experiment'}
            </p>
          </div>
        ) : (
          filtered.map(exp => (
